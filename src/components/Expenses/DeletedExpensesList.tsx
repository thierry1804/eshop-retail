...
const DeletedExpensesList: React.FC<DeletedExpensesListProps> = ({ onRestore }) => {
  ...
  const handlePermanentDelete = async (expenseId: string) => {
    ...
    try {
      ...
      // Essayer plusieurs méthodes de suppression définitive
      let success = false;
      let lastError = null;

      // Méthode 1: Fonction RPC principale
      try {
        const { data, error } = await supabase
        ...
        if (!error && data === true) {
          success = true;
          console.log('Suppression définitive réussie avec permanently_delete_expense');
        } else {
          lastError = error;
          console.log('Échec de permanently_delete_expense:', error);
        }
      } catch (err) {
        lastError = err;
        console.log('Erreur avec permanently_delete_expense:', err);
      }

      // Méthode 2: Fonction RPC alternative
      if (!success) {
        try {
          const { data, error } = await supabase
            .rpc('permanently_delete_expense_v2', { expense_id: expenseId });

          if (!error && data === true) {
            success = true;
            console.log('Suppression définitive réussie avec permanently_delete_expense_v2');
          } else {
            lastError = error;
            console.log('Échec de permanently_delete_expense_v2:', error);
          }
        } catch (err) {
          lastError = err;
          console.log('Erreur avec permanently_delete_expense_v2:', err);
        }
      }

      // Méthode 3: Fonction vraiment définitive
      if (!success) {
        try {
          const { data, error } = await supabase
            .rpc('really_delete_expense', { expense_id: expenseId });

          if (!error && data === true) {
            success = true;
            console.log('Suppression définitive réussie avec really_delete_expense');
          } else {
            lastError = error;
            console.log('Échec de really_delete_expense:', error);
          }
        } catch (err) {
          lastError = err;
          console.log('Erreur avec really_delete_expense:', err);
        }
      }

      // Méthode 4: Suppression directe (dernier recours)
      if (!success) {
        try {
          console.log('Tentative de suppression directe...');
          
          // D'abord vérifier que la dépense existe
          ...
          if (checkError) {
            setError('Dépense non trouvée ou déjà supprimée');
            return;
          }
          ...
          if (!deleteError) {
            ...
            console.log('Échec de la suppression directe:', deleteError);
          }
        } catch (err) {
          lastError = err;
          console.log('Erreur avec la suppression directe:', err);
        }
      }

      if (!success) {
        console.error('Toutes les méthodes de suppression ont échoué. Dernière erreur:', lastError);
        setError(`Impossible de supprimer définitivement la dépense. Erreur: ${lastError?.message || 'Inconnue'}`);
        return;
      }
      ...
      setError('Erreur inattendue lors de la suppression définitive');
    } finally {
      setLoading(false);
    }
  };
